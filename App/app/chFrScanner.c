...
void CHFRSCANNER_Found(void) {
    if (gEeprom.SCAN_RESUME_MODE > 80) {
        if (!gScanPauseMode) {
            gScanPauseDelayIn_10ms =
                scan_pause_delay_in_5_10ms * (gEeprom.SCAN_RESUME_MODE - 80) * 5;
            gScanPauseMode = true;
        }
    } else {
    ...
    if(gEeprom.SCAN_RESUME_MODE > 1 && gEeprom.SCAN_RESUME_MODE < 26)
    {
        if (!gScanPauseMode)
        {
            gScanPauseDelayIn_10ms = scan_pause_delay_in_5_10ms * (gEeprom.SCAN_RESUME_MODE - 1) *
    5; gScheduleScanListen    = false; gScanPauseMode         = true;
        }
    }
    else
    {
...
static void NextMemChannel(void) {
    ...
    const bool enabled = (gEeprom.SCAN_LIST_DEFAULT > 0 && gEeprom.SCAN_LIST_DEFAULT < 4)
                             ...
                             : true;
    const int chan1 = (gEeprom.SCAN_LIST_DEFAULT > 0 && gEeprom.SCAN_LIST_DEFAULT < 4)
                          ? gEeprom.SCANLIST_PRIORITY_CH1[gEeprom.SCAN_LIST_DEFAULT - 1]
                          : -1;
    const int chan2 = (gEeprom.SCAN_LIST_DEFAULT > 0 && gEeprom.SCAN_LIST_DEFAULT < 4)
                          ? gEeprom.SCANLIST_PRIORITY_CH2[gEeprom.SCAN_LIST_DEFAULT - 1]
                          : -1;
    ...
    if (!enabled || chan == 0xff) {
        chan = RADIO_FindNextChannel(gNextMrChannel + gScanStateDir, gScanStateDir, true,
    ...
    gScanPauseDelayIn_10ms =
        9; // 90ms .. <= ~60ms it misses signals (squelch response and/or PLL lock time) ?
