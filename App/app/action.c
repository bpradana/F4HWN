...
static void ACTION_AlarmOr1750(const bool b1750) {
  ...
  const AlarmState_t alarm_mode = (gEeprom.ALARM_MODE == ALARM_MODE_TONE)
                                      ? ALARM_STATE_TXALARM
...
void ACTION_Ptt(void) { gSetting_set_ptt_session = !gSetting_set_ptt_session; }
...
void ACTION_Wn(void) {
  ...
  if (FUNCTION_IsRx()) {
    ...
    if (gRxVfo->CHANNEL_BANDWIDTH == BANDWIDTH_NARROW &&
        gSetting_set_nfm == 1) {
      narrower = 1;
    }
