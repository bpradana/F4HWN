...
void MENU_AcceptSetting(void) {
    ...
    case MENU_BATCAL: { // voltages are averages between discharge curves of 1600 and 2200 mAh
        // gBatteryCalibration[0] = (520ul * gSubMenuSelection) / 760;  // 5.20V empty, blinking
        // above this value, reduced functionality below gBatteryCalibration[1] = (689ul *
        // gSubMenuSelection) / 760;  // 6.89V,  ~5%, 1 bars above this value gBatteryCalibration[2]
        // = (724ul * gSubMenuSelection) / 760;  // 7.24V, ~17%, 2 bars above this value
        gBatteryCalibration[3] = gSubMenuSelection; // 7.6V,  ~29%, 3 bars above this value
        // gBatteryCalibration[4] = (771ul * gSubMenuSelection) / 760;  // 7.71V, ~65%, 4 bars above
        // this value gBatteryCalibration[5] = 2300;
    ...
    case MENU_F2LONG:
    case MENU_MLONG: {
        uint8_t *fun[] = {&gEeprom.KEY_1_SHORT_PRESS_ACTION, &gEeprom.KEY_1_LONG_PRESS_ACTION,
                          &gEeprom.KEY_2_SHORT_PRESS_ACTION, &gEeprom.KEY_2_LONG_PRESS_ACTION,
                          &gEeprom.KEY_M_LONG_PRESS_ACTION};
        *fun[UI_MENU_GetCurrentMenuId() - MENU_F1SHRT] =
            gSubMenu_SIDEFUNCTIONS[gSubMenuSelection].id;
    } break;
...
void MENU_ShowCurrentSetting(void) {
    ...
    case MENU_SLIST3:
        gSubMenuSelection =
            RADIO_FindNextChannel(0, 1, true, UI_MENU_GetCurrentMenuId() - MENU_SLIST1 + 1);
        break;
...
static void MENU_Key_0_to_9(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld) {
    ...
    if (UI_MENU_GetCurrentMenuId() == MENU_MEM_CH || UI_MENU_GetCurrentMenuId() == MENU_DEL_CH ||
...
static void MENU_Key_MENU(const bool bKeyPressed, const bool bKeyHeld) {
    ...
    if (!gIsInSubMenu) {
        if (UI_MENU_GetCurrentMenuId() == MENU_UPCODE || UI_MENU_GetCurrentMenuId() == MENU_DWCODE)
            return;
#if 1
        if (UI_MENU_GetCurrentMenuId() == MENU_DEL_CH ||
...
static void MENU_Key_UP_DOWN(bool bKeyPressed, bool bKeyHeld, int8_t Direction) {
    ...
    if (UI_MENU_GetCurrentMenuId() == MENU_MEM_NAME && gIsInSubMenu &&
        edit_index >= 0) { // change the character
    ...
    if (!gIsInSubMenu) {
        ...
        if (UI_MENU_GetCurrentMenuId() != MENU_ABR && UI_MENU_GetCurrentMenuId() != MENU_ABR_MIN &&
...
void MENU_ProcessKeys(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld) {
    ...
    if (gScreenToDisplay == DISPLAY_MENU) {
        if (UI_MENU_GetCurrentMenuId() == MENU_VOL || UI_MENU_GetCurrentMenuId() == MENU_BATCAL) {
