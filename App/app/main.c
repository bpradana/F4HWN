    ...
    case KEY_1:
        if (!IS_FREQ_CHANNEL(gTxVfo->CHANNEL_SAVE)) {
            ...
            if (gScanStateDir != SCAN_OFF) {
                if (gCurrentFunction != FUNCTION_INCOMING || gRxReceptionMode == RX_MODE_NONE ||
            ...
            if (IS_MR_CHANNEL(
                    gEeprom.ScreenChannel[vfo])) { // copy channel to VFO, then swap to the VFO
    ...
    if (!gWasFKeyPressed) { // F-key wasn't pressed
        ...
        if (IS_FREQ_CHANNEL(gTxVfo->CHANNEL_SAVE)) { // user is entering a frequency
            ...
            if (Frequency >= BX4819_band1.upper &&
                ...
                const uint32_t center = (BX4819_band1.upper + BX4819_band2.lower) / 2;
                Frequency = (Frequency < center) ? BX4819_band1.upper - gTxVfo->StepFrequency
                                                 : BX4819_band2.lower;
            }
...
static void MAIN_Key_STAR(bool bKeyPressed, bool bKeyHeld) {
    ...
    {
        if (gScanStateDir == SCAN_OFF && gScanRangeStart == 0) { // start entering a DTMF string
            gBeepToPlay = BEEP_1KHZ_60MS_OPTIONAL;
