...
static void convertTime(uint8_t *line, uint8_t type) {
    ...
    gStatusLine[0] = gStatusLine[7] = gStatusLine[14] =
        0x00; // Quick fix on display (on scanning I, II, etc.)
...
void UI_DisplayStatus() {
        ...
        if (gCurrentFunction == FUNCTION_TRANSMIT && gSetting_set_tmr == true) {
            ...
            if (gEeprom.MENU_LOCK == true) {
                ...
                if (dw == 1 || dw == 3) { // DWR - dual watch + respond
                    if (gDualWatchActive)
                        memcpy(line + x + (dw == 1 ? 0 : 2), gFontDWR,
    ...
    case 1: // voltage
        const uint16_t voltage =
            (gBatteryVoltageAverage <= 999) ? gBatteryVoltageAverage : 999; // limit to 9.99V
