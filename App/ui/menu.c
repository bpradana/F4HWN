...
const char gSubMenu_TXP[][6] = {"USER", "LOW 1", "LOW 2", "LOW 3", "LOW 4", "LOW 5", "MID", "HIGH"};

const char gSubMenu_SFT_D[][4] = {"OFF", "+", "-"};

const char gSubMenu_W_N[][7] = {"WIDE", "NARROW"};

const char gSubMenu_OFF_ON[][4] = {"OFF", "ON"};

const char gSubMenu_NA[4] = {"N/A"};
...
const char *const gSubMenu_MDF[] = {"FREQ", "CHANNEL\nNUMBER", "NAME", "NAME\n+\nFREQ"};

const char gSubMenu_AL_MOD[][5] = {"SITE", "TONE"};


const char *const gSubMenu_PTT_ID[] = {"OFF", "UP CODE", "DOWN CODE", "UP+DOWN\nCODE",
                                       "APOLLO\nQUINDAR"};

const char gSubMenu_PONMSG[][8] = {"ALL", "SOUND", "MESSAGE", "VOLTAGE", "NONE"};

const char gSubMenu_ROGER[][6] = {"OFF", "ROGER", "MDC"};

const char gSubMenu_RESET[][4] = {"VFO", "ALL"};
...
const char gSubMenu_RX_TX[][6] = {"OFF", "TX", "RX", "TX/RX"};

const char gSubMenu_BAT_TXT[][8] = {"NONE", "VOLTAGE", "PERCENT"};

const char gSubMenu_BATTYP[][12] = {"1600mAh K5", "2200mAh K5", "3500mAh K5", "1400mAh K1",
                                    "2500mAh K1"};


const char gSubMenu_SET_PWR[][6] = {"< 20m", "125m", "250m", "500m", "1", "2", "5"};

const char gSubMenu_SET_PTT[][8] = {"CLASSIC", "ONEPUSH"};

const char gSubMenu_SET_TOT[][7] = // Use by SET_EOT too
    {"OFF", "SOUND", "VISUAL", "ALL"};

const char gSubMenu_SET_LCK[][9] = {"KEYS", "KEYS+PTT"};

const char gSubMenu_SET_MET[][8] = {"TINY", "CLASSIC"};

const char gSubMenu_SET_NFM[][9] = {"NARROW", "NARROWER"};

const char gSubMenu_SET_KEY[][9] = {"KEY_MENU", "KEY_UP", "KEY_DOWN", "KEY_EXIT", "KEY_STAR"};
...
// edit box
char edit_original[17]; // a copy of the text before editing so that we can easily test for
                        // changes/difference
char edit[17];
int edit_index;
...
void UI_DisplayMenu(void)
{
    ...
    UI_DrawLineBuffer(gFrameBuffer, 48, 0, 48, 55,
                      1); // Be ware, status zone = 8 lines, the rest = 56 ->total 64
    ...
    {                                       // new menu layout .. experimental & unfinished
        ...
        if (!gIsInSubMenu) {
            while (i < 2) { // leading menu items - small text
                ...
                if (k < 0)
                    UI_PrintStringSmallNormal(MenuList[gMenuListCount + k].name, 0, 0,
                                              i); // wrap-a-round
            ...
            while (i < 4) { // trailing menu item - small text
                const int k = menu_index + i - 2;
    ...
    case MENU_TXP:
        if (gSubMenuSelection == 0) {
            ...
            sprintf(String, "%s\n%sW", gSubMenu_TXP[gSubMenuSelection],
    ...
    case MENU_OFFSET:
        if (!gIsInSubMenu || gInputBoxIndex == 0) {
            sprintf(String, "%3d.%05u", gSubMenuSelection / 100000,
                    abs(gSubMenuSelection) % 100000);
    ...
    case MENU_AUTOLK:
        if (gSubMenuSelection == 0)
            ...
            sprintf(String, "%02dm:%02ds", ((gSubMenuSelection * 15) / 60),
                    ((gSubMenuSelection * 15) % 60));
            // #if !defined(ENABLE_SPECTRUM) || !defined(ENABLE_FMRADIO)
    ...
    case MENU_DEL_CH: {
        ...
        if (valid && !gAskForConfirmation) { // show the frequency so that the user knows the
                                             // channels frequency
    ...
    case MENU_MEM_NAME: {
        ...
        if (valid) {
            ...
            if (edit_index < 0) { // show the channel name
                ...
                if (edit_index < 10)
                    // UI_PrintString("^", menu_item_x1 + (8 * edit_index), 0, 4, 8);  // show the
                    // cursor
                    UI_PrintString("^", menu_item_x1 - 1 + (8 * edit_index), 0, 4,
                                   8); // show the cursor
            }

            if (!gAskForConfirmation) { // show the frequency so that the user knows the channels
                                        // frequency
                sprintf(String, "%u.%05u", frequency / 100000, frequency % 100000);
                UI_PrintString(String, menu_item_x1, menu_item_x2,
                               4 + (gIsInSubMenu && edit_index >= 0), 8);
            }
        }
    ...
    case MENU_TOT:
        sprintf(String, "%02dm:%02ds", (((gSubMenuSelection + 1) * 5) / 60),
                (((gSubMenuSelection + 1) * 5) % 60));
        // #if !defined(ENABLE_SPECTRUM) || !defined(ENABLE_FMRADIO)
    ...
    case MENU_RP_STE:
        sprintf(String, gSubMenuSelection == 0 ? gSubMenu_OFF_ON[0] : "%u*100ms",
                gSubMenuSelection);
        break;
    ...
    case MENU_SET_PWR:
        sprintf(String, "%s\n%sW", gSubMenu_TXP[gSubMenuSelection + 1],
        ...
        if (len > 0) {
            ...
            */
            ...
            if (UI_MENU_GetCurrentMenuId() == MENU_VOL) {
                sprintf(edit, "%u.%02uV %u%%", gBatteryVoltageAverage / 100,
        ...
        if (gSubMenuSelection < 0 || !gEeprom.SCAN_LIST_ENABLED[i]) {
            ...
            */
            ...
            for (uint8_t pri = 1; pri <= 2; pri++) {
                uint8_t channel = (pri == 1) ? gEeprom.SCANLIST_PRIORITY_CH1[i]
                                             : gEeprom.SCANLIST_PRIORITY_CH2[i];
    ...
    if (UI_MENU_GetCurrentMenuId() == MENU_R_CTCS || UI_MENU_GetCurrentMenuId() == MENU_T_CTCS ||
        UI_MENU_GetCurrentMenuId() == MENU_R_DCS || UI_MENU_GetCurrentMenuId() == MENU_T_DCS) {
        sprintf(String, "%2d", gSubMenuSelection);
        UI_PrintStringSmallNormal(String, 105, 0, 0);
    }

    if ((UI_MENU_GetCurrentMenuId() == MENU_RESET || UI_MENU_GetCurrentMenuId() == MENU_MEM_CH ||
         UI_MENU_GetCurrentMenuId() == MENU_MEM_NAME ||
         UI_MENU_GetCurrentMenuId() == MENU_DEL_CH) &&
        gAskForConfirmation) { // display confirmation
